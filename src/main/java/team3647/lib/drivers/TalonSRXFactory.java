...
public class TalonSRXFactory {
    ...
    public static class Configuration {
        ...
        public Configuration currentLimiting(
                boolean enable, int peakCurrent, int peakCurrentDuration, int continuousCurrent) {
    ...
    public static TalonSRX createTalon(Configuration config) {
        ...
        handleCANError(
                config.CANID, talon.configFactoryDefault(kTimeoutms), "restore factory defaults");
        talon.clearStickyFaults(kTimeoutms);

        talon.enableCurrentLimit(config.enableCurrentLimiting);
        handleCANError(
                config.CANID,
                talon.configPeakCurrentLimit(config.peakCurrent, kTimeoutms),
                "set peak current");
        handleCANError(
                config.CANID,
        ...
        handleCANError(
                config.CANID,
                talon.configPeakOutputForward(config.maxOutput, kTimeoutms),
                "set max forward output");

        handleCANError(
                config.CANID,
                talon.configPeakOutputReverse(config.minOutput, kTimeoutms),
